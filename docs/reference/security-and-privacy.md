...
桌面端使用一套不带 Zepp cookie jar 的独立 HTTPS client，只向 `https://zeppbridge.pages.dev/api/feedback` 发送强类型白名单报告。允许字段只有应用/解析器版本、操作系统、数据库 schema 版本、设备响应的字段名与 JSON 类型、目录候选、固件版本、安全的产品名/短型号提示、未知设备数量、未知运动编号与记录数、类型冲突数。报告类型没有账号、Token、cookie、序列号、设备 ID、GPS、健康数值、原始响应或本机路径的字段槽位。
