...
Rust library 测试只保留会挡住假成功、丢数据、错文案的门禁（约 20 个）：凭据不落盘、host 校验、同步 outcome、REM 不编造、保留天数边界、去重。不要为了数量再堆冗余用例。具体数量以当前 `cargo test` 输出为准。

## 当前 command 契约

Tauri command 在 `src-tauri/src/lib.rs` 注册，前端封装在 `src/composables/useTauriApi.ts`：
...
| `clear_auth` | 停止代理并清除认证 | 保留健康数据库 |
| `start_capture` | 启动局域网 HTTP/HTTPS 捕获 | 端口 `1024–65535`；返回 LAN IP、端口和证书 URL |
| `get_capture_status` | 读取捕获状态 | 只返回脱敏用户摘要，不返回 token |
| `stop_capture` | 停止监听器 | 等待任务退出并释放端口；CA 私钥留在 `com.zeppbridge.ca` 供后续复用 |
...
2. `ZeppConnector` 只构造 HTTPS origin，host 仅允许 `api-mifit*.zepp.com` / `api-mifit*.huami.com`，HTTP client 超时 30 秒，401/403/404/429/5xx 分类处理。
3. `DataFetcher` 为每个响应保留 stream/source key/raw payload。连接器有有限重试，但没有通用的 cursor 分页实现；运动 endpoint 使用 track ID 语义，当前窗口 helper 仍是保守范围。
4. `Normalizer` 只接受能识别的结构化数组/对象，并能解码当前真实 fixture 验证过的 Base64 `band_data` 睡眠/分钟心率结构；无法识别的编码仍只保留 raw 并标记 `unverified`。
5. `Database` 使用 WAL、外键和 schema migration（当前版本 1→2）；表达式唯一索引处理 `NULL device_id`，canonical 行保留 `raw_record_id`。
6. `SyncManager` 用 run lock 防止并发同步；核心流失败时 `success=false`，可选流显示 `unavailable`/`unverified`，成功后再做 retention。

## 前端开发约定

- 页面通过 `tauriApi` 调用 command，不直接访问 Zepp。
- 空值显示 `—`、`未记录` 或明确的空状态；不要把缺失数据变成 `0`。
- 时间格式化前检查 `Date.getTime()`；错误应保留可操作信息，不要静默吞掉字符串。
- 使用现有主题变量、`focus-visible`、语义元素、ARIA 和最小 44px 触控区域；移动端断点目前以 760px 为主。
